...
$clavesEstados = array_keys($estadosPedido);
$indiceActual = array_search($estadoActual, $clavesEstados);
...
<body>
    ...
    <section class="containerSeguimiento">
        <div class="containerSeguimientoInner">
            ...
            <div class="containerDetailsOrder">
                <iconify-icon icon="lets-icons:check-fill" class="iconcheckSeguimiento"></iconify-icon>
                <p><strong>Hora de registro:</strong> <?= htmlspecialchars($horaFormateada) ?> hrs</p>
            </div>

            <div class="mt-2">
                <p class="m-0 p-0"><strong>Progreso:</strong></p>
                <div class="secuenciaEstado">
                    <?php $i = 0; foreach ($estadosPedido as $clave => $nombre): ?>
                        <div class="<?= ($i <= $indiceActual) ? 'estadoActivo' : 'estadoPendiente' ?>">
                            <?= $nombre ?>
                        </div>
                    <?php
                        $i++;
